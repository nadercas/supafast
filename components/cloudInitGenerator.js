...
IMAGE_MANAGEMENT=ghcr.io/nadercas/supafast:v1.1.0
${enableS3Backups ? `
############
# Backups (S3)
############
...
BACKUP_START=$(date +%s)
BACKUP_START_ISO=$(date -Iseconds)
DUMP_SIZE_BYTES=0
SNAPSHOT_ID=""
...
write_status() {
  ...
  printf '{"success":%s,"timestamp":"%s","started":"%s","duration_seconds":%s,"dump_size_bytes":%s,"snapshot_id":"%s","error":"%s","server_name":"%s"}\\n' \\
    "\$success" "$(date -Iseconds)" "\$BACKUP_START_ISO" "\$elapsed" "\$DUMP_SIZE_BYTES" "\$SNAPSHOT_ID" "\$error" "\${SERVER_NAME:-unknown}" > "\$STATUS_FILE"
}
...
# Dump postgres
if ! docker exec "\${POSTGRES_CONTAINER:-supabase-db}" pg_dumpall -U "\${POSTGRES_USER:-postgres}" --clean 2>/dev/null | gzip > "$STAGING/postgres_dump.sql.gz"; then
  write_status false "pg_dumpall failed"
  [ -n "\${HEALTHCHECK_URL:-}" ] && curl -fsS -m 10 --retry 3 "\${HEALTHCHECK_URL}/fail" >/dev/null 2>&1 || true
  exit 1
fi
DUMP_SIZE_BYTES=$(stat -c%s "$STAGING/postgres_dump.sql.gz" 2>/dev/null || stat -f%z "$STAGING/postgres_dump.sql.gz" 2>/dev/null || echo 0)
...
# Backup
if ! RESTIC_OUT=$(restic backup --tag supabase --tag "\${SERVER_NAME}" --host "\${SERVER_NAME}" --json "\${PATHS[@]}" 2>&1 | tail -1); then
  write_status false "restic backup failed"
  [ -n "\${HEALTHCHECK_URL:-}" ] && curl -fsS -m 10 --retry 3 "\${HEALTHCHECK_URL}/fail" >/dev/null 2>&1 || true
  exit 1
fi
SNAPSHOT_ID=$(echo "\$RESTIC_OUT" | grep -o '"snapshot_id":"[^"]*"' | cut -d'"' -f4 || true)
